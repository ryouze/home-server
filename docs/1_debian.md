...
I chose Debian as my server OS because it does not come with Snap like Ubuntu, and it is known for its stability.
...
Before I install anything, it is a good idea to tweak the UEFI while I still have a monitor connected.
...
4. In `Advanced`, I opened `Intel (R) Manageability` and set `Intel (R) Manageability Control` to `Disabled`. It is remote administration firmware for fleet management, which I do not need for a simple home server.
5. In `Power`, I set `After Power Loss` to `Power On` to ensure the PC auto-starts after a power cut, which is useful for servers. I also opened `Automatic Power On` and set `Wake on LAN` to `Automatic`.
...
7. In `Startup`, I opened `Primary Boot Sequence` and set the USB drive as the first boot device.
8. Finally, I went back to `Exit` and selected `Save Changes and Exit`.
...
1. After a restart, I was greeted with a TTY and logged in. The password generated by the password manager was very long and difficult to type, but security is important.
2. I ran the usual `sudo apt update` and `sudo apt full-upgrade`, but the system was already up to date. I also ran `sudo -v` to confirm that `sudo` was set up correctly, which it was, since I left the root password blank during installation.
...
2. In `Startup`, I opened `Primary Boot Sequence` and set the NVMe with Debian as the first device.
